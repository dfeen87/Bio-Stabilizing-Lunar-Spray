...
class CuringSimulator:
    ...
    def plot_curing_curves(
        ...
        ax2.legend()
        ax2.axhline(
            y=CuringConstants.MAX_BOND_STRENGTH,
            color="g",
            linestyle="--",
            alpha=0.3,
            label="Max strength",
...
def generate_cure_time_chart():
    ...
    standard_times = np.array([standard.calculate_cure_time(t) for t in temperatures])
...
def run_example():
    ...
    for temp in temps:
        ...
        print(
            f"  {temp:3d}°C | Standard: {std_time:5.1f} min | "
