...
class RegolithSample:
    ...
    def validate(self) -> bool:
        ...
        total = (self.sio2_percent + self.al2o3_percent + self.feo_percent +
                self.cao_percent + self.mgo_percent + self.tio2_percent + 
                self.others_percent)
...
def arrhenius_factor(temperature_c: float, 
                    activation_energy_kj_mol: float,
                    reference_temp_c: float = 25.0) -> float:
    """
...
def interpolate_linear(x: float, 
                      x_points: List[float], 
...
class UnitConverter:
    ...
    @staticmethod
    def ppm_to_mg_per_liter(ppm: float, 
                           molecular_weight: float = 1.0) -> float:
        """
...
def validate_temperature(temp_c: float, 
    ...
    if not min_temp <= temp_c <= max_temp:
        raise ValueError(
            f"Temperature {temp_c}°C outside valid range "
            f"[{min_temp}, {max_temp}]°C"
        )
    return True


def validate_pressure(pressure_psi: float,
                     min_pressure: float = 0.0,
                     max_pressure: float = 100.0) -> bool:
    """
...
def validate_concentration(conc_ppm: float,
                          nutrient: str = "Nutrient",
                          max_safe: float = 10000.0) -> bool:
    """
