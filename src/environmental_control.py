...
class AIEnvironmentalController:
    ...
    def check_alerts(self, sensors: DomeSensors, setpoints: EnvironmentalSetpoints) -> List[Tuple[AlertLevel, str]]:
        ...
        if sensors.o2_percent < 18.0:
            alerts.append((AlertLevel.EMERGENCY, 
    ...
    def calculate_temperature_control(self, 
                                     current: float, 
                                     setpoint: float, 
    ...
    def calculate_co2_control(self, 
                             current: float, 
                             setpoint: float, 
                             dt: float) -> float:
        """
    ...
    def calculate_lighting_control(self, 
                                   current_hour: float,
                                   photoperiod: float) -> float:
        """
    ...
    def plot_performance(self, save_path: Optional[str] = None):
        ...
        ax2.axhline(y=self.state.setpoints.humidity_percent, color='g', 
                   linestyle='--', label='Setpoint')
        ...
        ax3.axhline(y=self.state.setpoints.co2_ppm, color='g', 
                   linestyle='--', label='Setpoint')
...
def run_example():
    ...
    print(f"\nSetpoints:")
    print(f"  Temperature: {controller.state.setpoints.temperature_c}°C ± {controller.state.setpoints.temperature_tolerance}°C")
    print(f"  Humidity: {controller.state.setpoints.humidity_percent}% ± {controller.state.setpoints.humidity_tolerance}%")
    print(f"  CO₂: {controller.state.setpoints.co2_ppm} ppm ± {controller.state.setpoints.co2_tolerance} ppm")
